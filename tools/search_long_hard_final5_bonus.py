...
# Search spaces
LONG_HARD_MIN_VALUES = [30, 31, 32]
LONG_HARD_MAX_VALUES = [34, 35, 36]
FINAL_SOFT_BONUS_VALUES = [0.0, -0.03, -0.06, -0.09, -0.12]
FINAL_MEDIUM_BONUS_VALUES = [0.0, -0.02, -0.04, -0.06, -0.08]
...
def make_predictor(candidate):
    base_predict = race_simulator.predict_finishing_positions
    base_lap_time = race_simulator.lap_time
    ...
    def simulate_driver_with_bonus(race_config, strategy):
        ...
        if len(stints) >= 2:
            ...
            if (
                ...
                and candidate["long_hard_min"] <= prev_stint["length"] <= candidate["long_hard_max"]
                and final_stint["length"] == 5
            ):
                final_tire = normalize_compound(final_stint["tire"])
                if final_tire == "SOFT":
                    final_bonus = candidate["final_soft_bonus"]
                elif final_tire == "MEDIUM":
                    final_bonus = candidate["final_medium_bonus"]
        ...
        for lap in range(1, total_laps + 1):
            ...
            if stint_index == len(stints) - 1:
                lap_bonus = final_bonus

            total_time += base_lap_time(base_lap, current_tire, tire_age, track_temp) + lap_bonus
    ...
    return predict_with_bonus, base_predict
...
def evaluate_with_candidate(races, base_params, candidate):
    ...
    try:
        ...
        patched_predict, _ = make_predictor(candidate)
        race_simulator.predict_finishing_positions = patched_predict
...
def main():
    parser = argparse.ArgumentParser(
        description="Search a narrow long-HARD -> final-5 bonus rule on historical races."
    )
    ...
    for long_hard_min in LONG_HARD_MIN_VALUES:
        for long_hard_max in LONG_HARD_MAX_VALUES:
            if long_hard_min > long_hard_max:
                continue
            for final_soft_bonus in FINAL_SOFT_BONUS_VALUES:
                for final_medium_bonus in FINAL_MEDIUM_BONUS_VALUES:
                    candidate = {
                        "long_hard_min": long_hard_min,
                        "long_hard_max": long_hard_max,
                        "final_soft_bonus": final_soft_bonus,
                        "final_medium_bonus": final_medium_bonus,
                    }
